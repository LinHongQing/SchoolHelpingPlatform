...
public class ProblemtypeDaoImpl extends HibernateDaoSupport implements ProblemtypeDao {
	Session session;
	
	private static String getHql(TransferDbData transferDbData) {
		String preHql = "SELECT p FROM Problemtype AS p WHERE p.isvalid = " + Configurations.db_entry_valid;
	...
	private List<Problemtype> hibernateListProcess(TransferDbData transferDbData)
		...
		List<Problemtype> list = query.list();
		return list;
	}
	...
	private void hibernateSaveProcess(TransferDbData transferDbData)
		...
		for (Integer key : transferDbData.getValues().keySet()) {
			...
			case uid:
				...
				break;
			case name:
				problemType.setName(value);
				break;
			case createIp:
				problemType.setCreateip(value);
				break;
			case createTime:
				problemType.setCreatetime(Integer.valueOf(value));
				break;
			case createUserUid:
				Admin admin = new Admin();
				admin.setUid(value);
				problemType.setAdmin(admin);
				break;
			}
		}
	...
	@Override
	public List<Problemtype> select(TransferDbData transferDbData)
			throws HibernateException, HibernateSessionNotInitializedException {
		// TODO Auto-generated method stub
