...
public class AuthorizationInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation ai) throws Exception {
		...
		switch (role) {
		case Configurations.authorization_request_type_admin: {	// 管理员
			if (session.getAttribute(Configurations.session_online_user_key) == null) {
				session.setAttribute(Configurations.session_admin_authorization_key, Configurations.string_authorization_fail);
		...
		break;
		case Configurations.authorization_request_type_user:
		default: {	// 普通用户
			if (session.getAttribute(Configurations.session_online_user_key) == null) {
				session.setAttribute(Configurations.session_user_authorization_key, Configurations.string_authorization_fail);
				System.out.println("无登录信息, 授权失败");
				return ai.invoke();
			}
			try {
		...
		case Configurations.authorization_request_type_user: {
			...
			case PrivilegeCodeRequestPathStorage.is_authorized:
				session.setAttribute(Configurations.session_user_authorization_key, Configurations.string_authorization_success);
		...
		break;
		case Configurations.authorization_request_type_admin: {
			// 判断管理员是否成功授权
